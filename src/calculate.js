...
const screeners = ['electric_transmission_lines', 'highway', 'tsunami_zone', 'floodway']
...
function assignBin(value) {
  ...
  const thresholds = { q1: 0.25, q2: 0.5, q3: 0.75 }
...
const calculateValue = (field = 'ugb_pct_rank', rows = [], indicator_set) => {
  ...
  const displayStringObject = { 1: '', 2: '', 3: '', 4: '', 0: ' <br> ' }
  ...
  rows.forEach((row) => {
    ...
    const currentString = `${formattedVar}: ${percentageRange} <br>`
  ...
  Object.keys(displayStringObject)
    .sort((a, b) => b - a)
    .forEach((bin) => {
